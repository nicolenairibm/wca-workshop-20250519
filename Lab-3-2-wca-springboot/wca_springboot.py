...
Generate summary in table format with columns: Category, Recommendations, Priority, Effort Estimate."""
    ...
    try:
        summary_response = call_wca_api(summary_prompt)
...
def migrate_structs(
    ...
    try:
        ...
        # Check multiple indicators for a Struts application
        struts_indicators = {
            'struts_config': list(source_path.glob("**/struts-config.xml")),
            'struts_actions': list(source_path.glob("**/action/*.java")),
            'struts_forms': list(source_path.glob("**/form/*.java")),
            'struts_dependencies': [],  # Will check pom.xml or build.gradle
        }
        
        # Check for Struts dependencies in build files
        pom_files = list(source_path.glob("**/pom.xml"))
        gradle_files = list(source_path.glob("**/build.gradle"))
        
        for pom in pom_files:
            content = pom.read_text()
            if 'struts' in content.lower():
                struts_indicators['struts_dependencies'].append(pom)
                
        for gradle in gradle_files:
            content = gradle.read_text()
            if 'struts' in content.lower():
                struts_indicators['struts_dependencies'].append(gradle)
        
        # Count how many Struts indicators we found
        indicators_found = sum(1 for files in struts_indicators.values() if files)
        
        if indicators_found == 0:
            console.print("[red]No Struts application indicators found. Please ensure you're in a Struts project directory.[/red]")
            console.print("[yellow]Expected to find at least one of:[/yellow]")
            console.print("- struts-config.xml")
            console.print("- Java files in 'action' directory")
            console.print("- Java files in 'form' directory")
            console.print("- Struts dependencies in pom.xml/build.gradle")
            raise typer.Exit(code=1)
        else:
            console.print(f"[green]Found {indicators_found} Struts application indicators[/green]")
            if not struts_indicators['struts_config']:
                console.print("[yellow]Warning: No struts-config.xml found. Will attempt migration based on other indicators.[/yellow]")
...
def extract_code_block(response: str, language: str = None) -> str:
    """
    Extract code blocks from WCA API response.
    
    Args:
        response (str): Response from WCA API containing code blocks
        language (str, optional): Language to extract (e.g., 'java', 'html'). If None, extracts first code block.
    ...
    # Get the first matching code block
...
def migrate_jsp_to_thymeleaf(source_dir: Path, output_dir: Path):
    ...
    template_paths = [
        ...
        output_dir / "target/classes/templates"
    ]
    ...
    for html_file in html_files:
        try:
            ...
            # Get the relative path from either template directory
            relative_path = None
            for template_path in template_paths:
                try:
                    if template_path.exists():
                        relative_path = html_file.relative_to(template_path)
                        break
                except ValueError:
                    continue
                    
            if not relative_path:
                raise ValueError(f"Could not determine relative path for {html_file}")
